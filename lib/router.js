module.exports = {
  async getRouter(app, apiRouter, routes, controllers, getMiddlewares, Op) {
    for (const route in routes) {
      ...
      // [x] @pierre écrire plutôt if (routes[route].redirect) {...} puis if (routes[route].action) {...}
      ...
      if (routes[route].action) {
        ...
        switch (method) {
          case "GET":
            ...
            break;
          case "POST":
            app.post(path, controllerClass[action]);
            break;
          case "DELETE":
            app.delete(path, controllerClass[action]);
            break;
          case "API":
            ...
            app.use(path, pathMiddlewares, apiRouter);
            break;
